...
namespace HiSpin
{
    public class InputPaypalEmail : PopUI
    {
        ...
        public override void SetContent()
        {
            ...
            if (string.IsNullOrWhiteSpace(Save.data.allData.user_panel.first_name))
                firstnameInput.SetTextWithoutNotify("");
            else
                firstnameInput.SetTextWithoutNotify(Save.data.allData.user_panel.first_name);
            if (string.IsNullOrWhiteSpace(Save.data.allData.user_panel.last_name))
                lastnameInput.SetTextWithoutNotify("");
            else
                lastnameInput.SetTextWithoutNotify(Save.data.allData.user_panel.last_name);
            if (string.IsNullOrWhiteSpace(Save.data.allData.user_panel.user_paypal))
                paypalemailInput.SetTextWithoutNotify("");
            else
